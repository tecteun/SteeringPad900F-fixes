...
This fork removes some of the features from the original project (multi-axis support, angle in FFB calculations), but fixes FFB related bugs. Especially racing-game related, as I am actively using this library to drive the FFB of a steering wheel.

I originally intented to backport some of my fixes into the original project, but I received no answer to my PR. Since then, I've made some major changes to the way force feedback is calculated. Merging back changes to the original repository is now improbable. 
...
There is also Arduino Due support, although it is still in beta. Please note that [a custom core](https://github.com/bogdan-dumitrescu/ArduinoCore-sam) is needed for Arduino Due to function as a USB device. Unfortunately, there are still some bugs in the Arduino Due custom core (or in this library). The Due does get recognized as an FFB device, but it throws errors when being loaded by the Linux kernel. In some of the games I've tested, the FFB generated by an Arduino Due is also missing a lot of the details the Leonardo outputs. Depending how the game uses the device, it might work. I've currently abandoned attempts to improve Due support. So unless you're ready to put considerable amount of time into finding the issues, you should use this library with an Arduino Leonardo.
