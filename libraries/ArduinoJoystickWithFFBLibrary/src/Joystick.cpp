...
void Joystick_::forceCalculator(int32_t *forces)
{
	...
	for (int id = 0; id < MAX_EFFECTS; id++)
	{
		...
		{
			forces[0] += (int32_t)(getEffectForce(effect, m_gains[0], m_effect_params[0]));
			// effectCount++;
		}
	}
	...
	// if (forces[0] > 0 || forces[0] < 0 || effectCount > 0)
	// {
	// 	char data[100];
	// 	sprintf(data, "Calculated force of %5ld combininng %2d effects", forces[0], effectCount);
	// 	Serial.println(data);
	// }
}
...
int32_t Joystick_::SinForceCalculator(volatile TEffectState& effect) 
{
	...
	if (period != 0)
	{
		float angle = ((float)effect.elapsedTime / period) * 2 * PI + effect.phase;
		sine = sin(angle);
	}
